...
public interface DeviceRepository extends JpaRepository<Device, UUID> {
    ...
    // 2. IOT INGEST: Para buscar el dispositivo cuando el Edge envía datos (usa la MAC)
    // Spring crea la query: SELECT * FROM devices WHERE mac_address = ?
    Optional<Device> findByMacAddress(String macAddress);

    // 3. RELACIÓN: Para buscar todos los sensores instalados en un Vehículo específico
    // Como en tu entidad Device definimos 'private UUID vehicleId', aquí pasas el UUID directo.
    List<Device> findByVehicleId(UUID vehicleId);
